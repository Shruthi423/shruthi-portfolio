...
// Palette
const CREAM = "#FFFAF2"; // morning mist
const FOG = "rgba(255,250,242,0.72)"; // frosted veil, same RGB as CREAM
const FONT = "#963417"; // terracota — the big links
const CHROME = "#3b2a1c"; // top bar + tagline

const LINKS = [
  { label: "Work", href: "/", cursor: "The good stuff" },
...
const COLOR: Record<Animal, string> = {
...
function OverlayBody({ onClose }: { onClose: () => void }) {
  ...
  return (
    <motion.div
      ...
      style={{ backgroundColor: CREAM, color: CHROME }}
    >
      ...
      {/* Top bar */}
      <div className="relative z-30 flex h-16 shrink-0 items-center justify-between px-8">
        <button
          ref={closeRef}
          onClick={onClose}
          data-cursor-label="Close"
          className="font-mono text-caption-1 font-medium uppercase tracking-wide opacity-70 transition-opacity hover:opacity-100"
        >
          ✕&nbsp;&nbsp;Close
        </button>
        <span
          ref={wordmarkRef}
          className="text-lg lowercase tracking-tight"
          ...
          {ANIMALS.map((a) => {
            ...
            return (
              <button
                ...
                style={
                  active
                    ? { backgroundColor: "rgba(150,52,23,0.10)", boxShadow: `inset 0 0 0 2px ${COLOR[a]}` }
                    : { opacity: 0.5 }
                }
              >
                <svg viewBox="0 0 100 110" width="26" height="28" fill={active ? COLOR[a] : CHROME}>
                  {shapeChildren(a)}
                </svg>
              </button>
            );
...
export function MenuOverlay({
  ...
  return (
    <AnimatePresence>{open && <OverlayBody onClose={onClose} />}</AnimatePresence>
  );
}
